...
async function statusIntegrity(fix = false) {
	...
	console.log(
		`${colors.bright}${colors.blue}Status Integrity Check${colors.reset}`,
	);
	console.log(
		`${colors.bright}${colors.blue}════════════════════════════════════════${colors.reset}\n`,
	);
	...
	if (await fs.pathExists(BOLTS_DIR)) {
		...
		for (const boltDir of boltDirs) {
			...
			if (bolt) {
				...
				if (
					bolt.frontmatter.intent &&
					!intents.includes(bolt.frontmatter.intent)
				) {
	...
	for (const intent of intents) {
		if (await fs.pathExists(path.join(INTENTS_DIR, intent, "units"))) {
			const unitDirs = await fs
				.readdir(path.join(INTENTS_DIR, intent, "units"))
				.catch(() => []);
	...
	if (inconsistencies.length === 0) {
		console.log(
			`${colors.green}✓${colors.reset} All statuses are consistent!\n`,
		);
	} else {
		console.log(
			`${colors.bright}${colors.yellow}⚠️ ${inconsistencies.length} inconsistencies found${colors.reset}\n`,
		);
		...
		for (const issue of inconsistencies) {
			const shortPath = issue.path
				.replace(/^memory-bank\//, "")
				.replace(/^.*\/intents\//, "");
			console.log(
				`| ${shortPath} | ${issue.current} | ${issue.expected} | ${issue.reason} |`,
			);
		}
		...
		if (fix) {
			...
			for (const issue of inconsistencies) {
				...
				if (fixed) {
					console.log(
						`  ${colors.green}✓${colors.reset} Fixed: ${issue.path.replace(/^memory-bank\//, "")}`,
					);
					fixedItems.push(issue);
				} else {
					console.log(
						`  ${colors.red}✗${colors.reset} Failed: ${issue.path.replace(/^memory-bank\//, "")}`,
					);
				}
			}
			...
			console.log();
			console.log(
				`${colors.green}Fixed ${fixedItems.length} of ${inconsistencies.length} inconsistencies${colors.reset}\n`,
			);
		} else {
			console.log(
				`${colors.dim}Run with --fix to automatically correct these issues.${colors.reset}\n`,
			);
		}
	}
	...
	console.log(
		`Inconsistencies: ${colors.red}${inconsistencies.length}${colors.reset}\n`,
	);
