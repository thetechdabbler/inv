...
class ArtifactValidator {
	...
	async validateNamingConventions() {
		console.log(
			`${colors.dim}[1/4] Validating naming conventions...${colors.reset}`,
		);
		let issues = 0;
	...
	async validateIdFilenameConsistency() {
		console.log(
			`${colors.dim}[2/4] Validating ID-filename consistency...${colors.reset}`,
		);
		let issues = 0;
			...
			for (const intentDir of intentDirs) {
				...
				if (await fs.pathExists(unitsDir)) {
					...
					for (const unitDir of unitDirs) {
						const storiesDir = path.join(unitsDir, unitDir, "stories");
						if (await fs.pathExists(storiesDir)) {
							const storyFiles = (await fs.readdir(storiesDir)).filter(
								(f) => path.extname(f) === ".md",
							);
							for (const storyFile of storyFiles) {
	...
	async validateCrossReferences() {
		console.log(
			`${colors.dim}[3/4] Validating cross-references...${colors.reset}`,
		);
		let issues = 0;
	...
	async validateTimestamps() {
		console.log(
			`${colors.dim}[4/4] Validating timestamp formats...${colors.reset}`,
		);
		let issues = 0;
	...
	async validateAll() {
		...
		console.log(
			`${colors.bright}${colors.blue}Artifact Validator${colors.reset}`,
		);
		console.log(
			`${colors.bright}${colors.blue}════════════════════════════════════════${colors.reset}\n`,
		);
		...
		for (const result of this.results) {
			...
			if (result.rule === "story.id-matches-filename") {
				...
				if (frontmatter) {
					...
					if (newContent) {
						...
						console.log(
							`  ${colors.green}✓${colors.reset} Fixed story ID: ${result.file}`,
						);
						fixed++;
					}
				}
			...
			if (result.rule === "bolt.id-matches-folder") {
				...
				if (frontmatter) {
					...
					if (newContent) {
						await fs.writeFile(result.file, newContent, "utf8");
						console.log(
							`  ${colors.green}✓${colors.reset} Fixed bolt ID: ${result.file}`,
						);
						fixed++;
					}
				}
			...
			if (result.rule === "timestamp.no-milliseconds") {
				...
				if (frontmatter && frontmatter[result.field]) {
					frontmatter[result.field] = frontmatter[result.field].replace(
						/\.\d+Z$/,
						"Z",
					);
					const newContent = updateFrontmatter(content, frontmatter);
					if (newContent) {
						await fs.writeFile(result.file, newContent, "utf8");
						console.log(
							`  ${colors.green}✓${colors.reset} Fixed timestamp: ${result.file}`,
						);
						fixed++;
					}
				}
	...
	toConsole() {
		...
		if (this.results.length === 0) {
			...
			console.log(
				`${colors.bright}${colors.yellow}⚠ ${this.results.length} issues found${colors.reset}`,
			);
			console.log(
				`${colors.dim}(${errors} errors, ${warnings} warnings)${colors.reset}\n`,
			);
			...
			for (const [type, issues] of Object.entries(byType)) {
				console.log(
					`${colors.bright}${type.toUpperCase()} Issues:${colors.reset}\n`,
				);
