...
function extractFrontmatter(content) {
	...
	try {
		...
		console.error(
			`${colors.red}Error parsing YAML frontmatter:${colors.reset}`,
			error.message,
		);
		return null;
	}
...
async function updateStories(bolt) {
	...
	for (const storyId of stories) {
		...
		if (!storyPath) {
			console.log(
				`  ${colors.red}✗${colors.reset} ${storyId} - ${colors.dim}File not found${colors.reset}`,
			);
			results.errors++;
		...
		if (!frontmatter) {
			console.log(
				`  ${colors.red}✗${colors.reset} ${storyId} - ${colors.dim}Invalid frontmatter${colors.reset}`,
			);
			results.errors++;
			results.details.push({
				storyId,
		...
		if (frontmatter.status === "complete" && frontmatter.implemented === true) {
			console.log(
				`  ${colors.dim}−${colors.reset} ${storyId} - ${colors.dim}Already complete${colors.reset}`,
			);
			results.skipped++;
			results.details.push({
				storyId,
		...
		if (newContent) {
			...
			console.log(
				`  ${colors.green}✓${colors.reset} ${storyId} - ${colors.dim}${oldStatus} → complete${colors.reset}`,
			);
			results.updated++;
			results.details.push({
				storyId,
			...
			console.log(
				`  ${colors.red}✗${colors.reset} ${storyId} - ${colors.dim}Failed to update${colors.reset}`,
			);
			results.errors++;
			results.details.push({
				storyId,
...
async function updateUnitStatus(bolt) {
	...
	if (allComplete && currentStatus !== "complete") {
		...
		const incompleteCount = unitBolts.filter(
			(b) => b.status !== "complete",
		).length;
		console.log(
...
async function updateIntentStatus(bolt) {
	...
	if (!(await fs.pathExists(unitsDir))) {
		console.log(
			`${colors.dim}No units directory found for intent${colors.reset}`,
		);
		return { updated: false };
	}
	...
	if (!(await fs.pathExists(requirementsPath))) {
		console.log(
			`${colors.dim}Requirements file not found, skipping intent status update${colors.reset}`,
		);
		return { updated: false };
	}
	...
	if (!requirementsFrontmatter) {
		console.log(
			`${colors.dim}Invalid requirements frontmatter, skipping${colors.reset}`,
		);
		return { updated: false };
	}
	...
	if (unitsAllComplete && currentStatus !== "complete") {
		...
		if (newContent) {
			...
			console.log(
				`${colors.green}✓${colors.reset} Intent status: ${currentStatus} → complete`,
			);
			return { updated: true, from: currentStatus, to: "complete" };
		}
		...
		console.log(
			`${colors.dim}Intent has ${incompleteCount} incomplete unit(s), status unchanged${colors.reset}`,
		);
	}
...
function validateBoltStatus(bolt) {
	...
	if (status !== "in-progress") {
		return {
			valid: false,
			reason: `Bolt status is "${status}", expected "in-progress"`,
		};
	}
...
async function boltMarkComplete(boltId, lastStage) {
	...
	console.log(
		`${colors.bright}${colors.blue}Bolt Completion: ${boltId}${colors.reset}`,
	);
	console.log(
		`${colors.bright}${colors.blue}════════════════════════════════════════${colors.reset}\n`,
	);
	...
	try {
		...
		if (!validation.valid) {
			...
			console.error(
				`${colors.dim}Use bolt-status command to check current state.${colors.reset}`,
			);
			return 1;
		}
		...
		console.log(
			`${colors.dim}Intent: ${bolt.frontmatter.intent}${colors.reset}`,
		);
		console.log(`${colors.dim}Unit: ${bolt.frontmatter.unit}${colors.reset}`);
		console.log(
			`${colors.dim}Stories: ${(bolt.frontmatter.stories || []).length}${colors.reset}\n`,
		);

		// Step 2: Update bolt file to complete status
		await updateBoltStatus(bolt, lastStage);
		console.log(
			`${colors.green}✓${colors.reset} Bolt status: ${bolt.frontmatter.status || "in-progress"} → complete\n`,
		);
		...
		console.log(
			`${colors.bright}${colors.green}✓ Bolt Complete: ${boltId}${colors.reset}`,
		);
		console.log(
			`${colors.bright}${colors.blue}════════════════════════════════════════${colors.reset}`,
		);
...
if (!boltId) {
	...
	console.error(
		`${colors.dim}Usage: node bolt-complete.cjs <bolt-id> [--last-stage <stage-name>]${colors.reset}`,
	);
	process.exit(1);
}
