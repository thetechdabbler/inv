...
export async function PUT(request: Request) {
  try {
    ...
    const { financialYear, ratePercentPerAnnum } = body as Record<string, unknown>;
    if (
      ...
      return NextResponse.json(
        { code: "VALIDATION_ERROR", message: "financialYear must be an integer between 2000 and 2100" },
        { status: 400 },
      );
    }
    if (
